 ...
 *
 * Prompts the user for a name first so the My Albums list is meaningful
 * on their next visit. Pressing Cancel aborts the entire creation —
 * the user stays on the picker rather than landing in an album they
 * decided not to start. Leaving the field empty defaults to a dated
 * fallback like "Untitled · May 7" so the card is still distinguishable
 * from any other untitled drafts they have.
 */
function startNewAlbumAndNavigate(router: ReturnType<typeof useRouter>, path: string) {
  if (typeof window === 'undefined') return;

  const raw = window.prompt(
    'Name this album (you can rename later by editing the cover title):',
    '',
  );
  if (raw === null) return; // user pressed Cancel — don't create

  const trimmed = raw.trim();
  // Strip control characters and very long inputs — protects the JSON-
  // serialized index from accidental abuse via paste.
  const sanitized = trimmed.replace(/[\x00-\x1f\x7f]/g, '').slice(0, 80);
  const dated = (() => {
    try {
      const d = new Date();
      return 'Untitled · ' + d.toLocaleDateString(undefined, { month: 'short', day: 'numeric' });
    } catch {
      return 'Untitled';
    }
  })();
  const name = sanitized || dated;
  ...
  if (typeof w._folioUpsertAlbumIndex === 'function') {
    w._folioUpsertAlbumIndex(id, { name });
  }
  const sep = path.indexOf('?') >= 0 ? '&' : '?';
...
export default function DesignerPage() {
  ...
  /**
   * Rename an existing album — opens a prompt seeded with the current
   * name and writes the result back into the index. Once a user gives
   * an album a custom name here, the auto-name-from-cover-text logic
   * in album-builder.js will leave it alone (see the stillUntitled
   * guard there).
   */
  function renameAlbum(id: string) {
    const album = albums.find((a) => a.id === id);
    const current = album ? album.name : '';
    const raw = window.prompt('Rename album:', current);
    if (raw === null) return;
    const next = raw.trim().replace(/[ -]/g, '').slice(0, 80);
    if (!next || next === current) return;
    const w = window as unknown as {
      _folioUpsertAlbumIndex?: (id: string, patch: Record<string, unknown>) => void;
    };
    if (typeof w._folioUpsertAlbumIndex === 'function') {
      w._folioUpsertAlbumIndex(id, { name: next });
    } else {
      // Fallback: rewrite the index directly so the rename still works
      // even before the legacy script has loaded.
      try {
        const indexRaw = window.localStorage.getItem('folio-albums-index');
        const arr = indexRaw ? JSON.parse(indexRaw) : [];
        if (Array.isArray(arr)) {
          const idx = arr.find((a: AlbumIndexEntry) => a.id === id);
          if (idx) {
            idx.name = next;
            idx.lastEditedAt = new Date().toISOString();
            window.localStorage.setItem('folio-albums-index', JSON.stringify(arr));
          }
        }
      } catch {}
    }
                ...
                return (
                  ...
                  >
                    <div style={{
                      ...
                      color: 'var(--cream)',
                      marginBottom: 4,
                      paddingRight: 56, /* leaves room for both ✎ and × buttons in the corner */
                      whiteSpace: 'nowrap',
                      overflow: 'hidden',
                    ...
                    <button
                      type="button"
                      onClick={(e) => { e.stopPropagation(); renameAlbum(album.id); }}
